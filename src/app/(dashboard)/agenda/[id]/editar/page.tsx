...
export default async function EditarAgendamentoPage({
  ...
  const [{ data: ag }, { data: profs }, { data: pacs }] = await Promise.all([
    supabase
      .from("agendamentos")
      .select(
        "id, data_hora_inicio, data_hora_fim, status, observacoes, profissional_id, paciente_id"
      )
      .eq("id", params.id)
      .single(),
    supabase
    ...
    <div className="p-6 md:p-10 max-w-3xl">
      ...
      <form action={editAction} className="card space-y-5">
        <div>
          ...
          <select
            ...
            defaultValue={ag.profissional_id}
          >
            {(profs ?? []).map((p: any) => (
      ...
      {profile.role === "admin" && (
        <div className="mt-6 p-4 border border-rust/20 rounded-xl bg-rust/5">
          ...
          <p className="text-xs text-rust/70 mb-3">
            Esta ação é irreversível. O agendamento será excluído permanentemente.
          </p>
